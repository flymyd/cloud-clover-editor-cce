...
class CFPropertyList extends CFBinaryPropertyList implements Iterator {
  ...
  /**
   * flag stating if we need to treat hex values as integer
   * @var boolean
   */
  protected $hexAsInt = false;
   ...
   */
  public function __construct($file=null,$format=self::FORMAT_AUTO,$hexAsInt=false) {
    $this->file = $file;
    $this->format = $format;
    $this->detectedFormat = $format;
    $this->hexAsInt = $hexAsInt;
  ...
  protected function import(DOMNode $node, $parent) {
    ...
    foreach($node->childNodes as $n) {
      ...
      switch($n->nodeName) {
        ...
        case 'integer':
          $value = !$this->hexAsInt ?
                new $class($n->nodeName == 'real' ? floatval($n->nodeValue) : intval($n->nodeValue)) :
                new $class($n->nodeValue);
          break;
        case 'true':
        case 'false':
   ...
   */
  public function saveXML($file, $formatted=true) {
   ...
   */
  public function save($file=null,$format=null,$formatted_xml=true) {
    $file = $file ? $file : $this->file;
