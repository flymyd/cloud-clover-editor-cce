  ...
  public function toCFType($value) {
    switch(true) {
      ...
      case is_string($value):
        if (substr($value,0,4) === 'b64_' || strpos($value, "\x00") !== false)
          return new CFData(substr($value,4));

        if (($this->hexAsInt && substr($value,0,2) == '0x') || substr($value, 0,5) == 'fint_') {
            if (substr($value, 0,5) == 'fint_')
                $value = substr($value, 5);

            return new CFNumber($value, $this->hexAsInt);
        }
